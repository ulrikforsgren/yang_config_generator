...
#  - Use argparse:
#     - Part of model iterate
#     - Number of list entries to create.
...
def generate_random_data(datatype, schema, node):
  ...
  if dt == 'empty':
    ...
    if isinstance(n.parent, List) and n.name in n.parent.key_leafs:
      ...
      if g:
        return g(n.datatype)
    return generate_random_data(n.datatype, schema, n)
...
def iter_schema(ch, doc, path=None, schema=None):
  path = path or tuple()
  for k,t in ch:
    if ':' in k:
        m,k = k.split(':')
    ...
    if isinstance(t, Container):
      ...
      if t.module:
        ns = get_ns(t.module)
        e.set('xmlns', ns)
      iter_schema(t, e, path=tp, schema=schema)
        ...
        for _ in range(0, n):
            ...
            if g:
              ...
              for ln in t.key_leafs:
                  kl = t.children[ln]
                  ET.SubElement(e, ln).text = generate_random_data(kl.datatype, schema, kl)
            iter_schema(t, e, path=tp, schema=schema)
...
s = Schema(tree)
iter_schema(s, dev_config, schema=s)
