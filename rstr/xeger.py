...
class XegerMinMax(object):
    def __init__(
        ...
        self._cases: Mapping[str, Callable[..., Any]] = {
            ...
            'at': lambda x: (0, 0),  # Does this need to be ignored?
            'in': lambda x: self._handle_in(x),  # Can this always return (1, 1)?
            'any': lambda x: (1, 1),
            'range': lambda x: (1, 1),
    ...
    def _handle_state(self, state: Any) -> Any:
        ...
        if opcode == 'category':
            value = value.name.lower()
        result = self._cases[opcode](value)
        if opcode == 'in':
            assert(result == (1, 1))
        return result
    ...
    def _handle_in(self, value: Any) -> Any:
        ...
        if candidates[0] is False:  # Ignore negate
    ...
    def _handle_branch(self, value: Any) -> Any:  # Probably need to update this
        result = []
        for branch in value:
            bresult = [self._handle_state(i) for i in branch]
            result.append(functools.reduce(lambda a, b: (a[0] + b[0], a[1] + b[1]), bresult))
